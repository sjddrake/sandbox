 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class AudioBookFileRenamerPlugIn
	...
	public void processFilePreRecurse(String pFolderName, File fileInFolder) {
		...
		if (!fileInFolder.isDirectory()) {
			...
			if (includeFileByExtension(lFileExt.toString())) {
				...
				String reformatedName = formatAudioBookFilename(lFilename.toString());
	...
	private String formatAudioBookFilename(String filename) {
		String reformattedFilename = null;
		
		// filename format: 1-01 The Burning Land1a
...
//				notFinished = false;
		...
		if (digitFound) {
			reformattedFilename = filename.substring(0, offset+1);
			reformattedFilename = reformattedFilename.trim();
		}		
		
		System.out.println(filename+" > has been reformatted to > "+reformattedFilename);
		
		return reformattedFilename;
	}
